...
RSpec.describe Merchant do
  describe 'Relationships' do
    ...
    it {should have_many(:order_discounts).through(:discounts)}
  ...
  describe 'Instance Methods' do
    before :each do
      ...
      @ogre = @morgan.items.create!(name: 'Ogre', description: "I'm an Ogre!", price: 20.25, image: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTaLM_vbg2Rh-mZ-B4t-RSU9AmSfEEq_SN9xPP_qrA2I6Ftq_D9Qw', active: true, inventory: 5 )
      ...
      @user_1 = User.create!(name: 'Morgan', address: '123 Main St', city: 'Denver', state: 'CO', zip: 80218, email: 'morgan_1@example.com', password: 'securepassword')
      @user_2 = User.create!(name: 'Morgan', address: '123 Main St', city: 'Denver', state: 'IA', zip: 80218, email: 'morgan_2@example.com', password: 'securepassword')
    ...
    it '.item_count' do
      expect(@morgan.item_count).to eq(2)
      expect(@brian.item_count).to eq(1)
      expect(@sal.item_count).to eq(0)
    end

    it '.average_item_price' do
      expect(@morgan.average_item_price.round(2)).to eq(35.13)
      expect(@brian.average_item_price.round(2)).to eq(50.00)
    end

    it '.distinct_cities' do
      expect(@morgan.distinct_cities).to eq(['Denver, CO', 'Denver, IA'])
    end

    it '.pending_orders' do
      expect(@morgan.pending_orders).to eq([@order_1])
    end

    it '.order_items_by_order' do
      expect(@morgan.order_items_by_order(@order_1.id)).to eq([@order_item_1])
    end
  end
end
